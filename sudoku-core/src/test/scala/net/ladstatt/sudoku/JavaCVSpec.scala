 ...
 */
class JavaCVSpec extends AnyWordSpecLike with CanLog {
  ...
  /**
   * shows how to wipe off artifacts at the border of a sudoku cell.
   *
   * sometimes, there are unwanted artifacts at the border of a cell to investigate, leading to
   * false positives. one approach is to use 'connectedComponentWithStats' function. another one,
    ...
    /*
        if (notTouchingBorder.nonEmpty) {
          notTouchingBorder.foldLeft(opencv_core.notEquals(notTouchingBorder.head)) {
            case (m, i) => opencv_core.notEquals()
          }

        } else {
          // only labels were found which didn't touch border

        }
        notTouchingBorder.foldLeft
        val matexpr =
          for (i <- notTouchingBorder) yield {
            opencv_core.notEquals(labels, m)
          }

        notTouchingBorder.foldLeft(labels) {
          case (mat, i) => opencv_core
        }
    */
    //    JavaCV.writeMat(Sudoku.targetPath.resolve("labels.png"), labels)
    //    JavaCV.writeMat(Sudoku.targetPath.resolve("stats.png"), stats)
  ...
  /*
    ...
    /**
     * what happens if we detect a list of curves with only one element.
     */
  ...
  private def printByteBf(labels: Mat) = {
    ...
    val iBf = labels.createBuffer[ByteBuffer]
  ...
  private def printIntBf(labels: Mat) = {
    val width = labels.size().width()
    val height = labels.size().height()
    val iBf = labels.createBuffer[IntBuffer]
    for {h <- 0 until height} {
      for {w <- 0 until width} {
        print(iBf.get(width * h + w))
      }
