...
trait OpenCvUnitTest {
  ...
  lazy val (sudoku69Result,_,_) = time(Await.result(sudoku69.calc(SudokuState(cap = 1, minHits = 17),Map(),Map()), Duration.Inf), t => logInfo(s"emptyFrame: $t ms"))

  lazy val emptyFrame = new Mat(1280, 768, CvType.CV_8UC3)
  lazy val emptySudoku = SCandidate(nr = 0, frame = emptyFrame)
  lazy val (emptySudokuResult,_,_) = time(Await.result(emptySudoku.calc(SudokuState(cap = 1, minHits = 1),Map(),Map()), Duration.Inf), t => logInfo(s"emptyFrame: $t ms"))
